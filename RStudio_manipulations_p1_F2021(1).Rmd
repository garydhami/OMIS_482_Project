---
title: "RStudio Manipulations: Part 1"
...
knitr::opts_chunk$set(
  ...
  message= FALSE)
```
## RStudio Manipulations: Part 1

The following sections of our book R for Data Science( [class book](https://r4ds.had.co.nz) ) are included in the second class:

Chapter 1 Sections: 1.1:1.2; 1.4: 1.6; 
Chapter 4 
Chapter 5 Stop at Section 5.6.1 
Chapter 6 
Optional sections:  Chapter 2: outside the scope of this class; Chapter 7: not a foundation chapter for what we will cover in other classes;

### Link to other resources
Debugging, find solutions of errors: 
[RStudio Support](https://support.rstudio.com/hc/en-us)
[Stackoverflow](https://stackoverflow.com)

RStudio external materials: [RStudio Resources](https://resources.rstudio.com)

RStudio Cheat Sheet: [RStudio Cheat Sheets](https://rstudio.com/resources/cheatsheets/)


## Review

### Running code in RStudio
If you are running code interactively in the console you just need to enter commands after the prompt sign ">".
If you are running the code from this window (Script Editor) you can press Cmd(Mac)/Ctrl(Windows) + Enter to run a single line of code or highlight multiple line and then press Cmd/Ctrl + Enter.
...
```
#### Data type in R [review video about data types available on BlackBoard Week 1 module for more info]:
...
^above data types are the ones we will use most in this class^
...
The result is a new data frame (remember to assign it to a new object if you want to preserve the changes). Together these properties make it easy to chain together multiple simple steps to achieve a complex result.
...
```{r}
#Example: flights that traveled on January 1st (Note the # of observation included in the dataset)
filter(flights,month == 1,day == 1)#the results do not replace the original dataset. To save it to future analysis assign it to a new variable named jan1. Create the variable jan1 
 <- filter(flights, month == 1, day == 1)
#R either prints out the results, or saves them to a variable. If you want to do both, you can wrap the assignment in parentheses
#Example 2: flights that traveled for Christmas
(FlightsChristmas <- filter(flights,month == 12, day == 25)) #by putting code between parenthesis you will save the object and print the results
```
...
Multiple arguments to filter() are combined with “and”: every expression must be true in order for a row to be included in the output. For other types of combinations,

you’ll need to use Boolean operators yourself: & is “and”, | is “or”, and ! is “not”. The "," used above can be used instead of &. 

However, I do recommend to use & especially at the beginning as it is easier for you to remind that both conditions must be met for an observation to be included in the analysis.
...
#NO, there is no flight that left in november and in december

#Example 4: find flights in November or December
filter(flights,month == 11 | month == 12)# Now this makes sense as you want to constraint the focus of your analysis to the last two months of the year. You might have an intuition that during the holiday season there is need for more flights/personnel 
...
### Exercise 1: Filter (1 & 4 in class; 2 & 3 are homeworks) [write code just below each instuction, then paste the code in the Word class 2 activities file; finally  use Teams RStudio - Forum channel for help on the homeworks or if you have other questions] - 5 minutes
```{r}
...
filter(flights,dep_delay<15 | arr_delay<15)
#flights with distance equal or bigger than 1010 miles
filter(flights,distance >= 1010)
...
### Exercise 2: Arrange (1 & 4 in class; 2 & 3 are homeworks) [write code just below each instuction, then paste the code in the Word class 2 activities file; finally  use Teams RStudio - Forum channel for help on the homeworks or if you have other questions] - 5 minutes
```{r}
#flights by distance from smaller to bigger (increasing)
arrange(flights,distance);
#flights by distance from bigger to smaller (decreasing)
arrange(flights,desc(distance))
#flights by air time
...
### Exercise 3: Select (1 & 4 in class; 2 & 3 are homeworks) [write code just below each instuction, then paste the code in the Word class 2 activities file; finally  use Teams RStudio - Forum channel for help on the homeworks or if you have other questions] - 5 minutes
```{r}
...
##### Mutate()
**mutate()** enables you to change the columns available in your original dataset. By using mutate() you can add new columns that are functions of existing columns. For example, in the tax_payers dataset, you note that the salary column is reported in euros rather than dollars. In this case, you want to create a new column that reports salary in dollars. To do so you need to multiple the value that are in the original salary by the conversion rate between euros and dollars. --> **mutate(tax_payers, salary_USD = salary * 1.21)**. Note how I chose a meaningful name for the new column, and that the conversion rate at the time I created this rmd file was 1.21.
...
transmute(flights_sml,speed = distance/air_time*60)#If you only want to keep the new variables, use transmute()
```

### Exercise 4: Mutate (1 & 4 in class; 2 & 3 are homeworks) [write code just below each instuction, then paste the code in the Word class 2 activities file; finally  use Teams RStudio - Forum channel for help on the homeworks or if you have other questions] - 5 minutes
```{r}
#compute a variable that shows the difference between scheduled departure time and the actual time of departure
mutate(flights,diff_dep_time = sched_dep_time - dep_time)
transmute(flights,diff_dep_time = sched_dep_time - dep_time)
#compute a variable that shows the air time divided by the total flight distance
mutate(flights,variable = air_time/distance)
transmute(flights,variable = air_time/distance)
#compute a variable that shows the difference between scheduled arrival time and the actual time of arrival
mutate(flights,diff = sched_arr_time - arr_time)
transmute(flights,diff = sched_arr_time - arr_time)
##compute a variable that shows the sum of the departure delay and the air time
mutate(flights,Sum_time = dep_delay + air_time)
transmute(flights,Sum_time = dep_delay + air_time)
```

## Summarise()
**summarise()** enables you to compute descriptive statistics of your dataset. summarise() collapses a data frame to a single row. For example, if you want to compute the average salary of the observation of the tax payers in your dataset, summarise will return to you one row that contains the average value. --> **summarise(tax_payers, avg_salary = mean(salary))**. Note how I chose a meaningful name for the output of my summary.
...
So, when you use the group_by() function you can change the unit of analysis from the complete dataset to individual groups (columns that caught your attention). group_by will create a group for each unique value available in the selected column.Then, when you use the dplyr verbs on a grouped data frame they’ll be automatically applied “by group”. For example, if we applied exactly the same code to a data frame grouped by date, we get the average delay per date:
```{r}
...
summarise(by_carrier,avg_delay_airlines = mean(dep_delay,na.rm = T))#help in answer the question about how the airlines are affecting the delay
```
...
### Exercise 5: Summarise (1 & 4 in class; 2 & 3 are homeworks) [write code just below each instuction, then paste the code in the Word class 2 activities file; finally  use Teams RStudio - Forum channel for help on the homeworks or if you have other questions] - 5 minutes
```{r}
#compute the max dep_delay
summarise(flights,max(dep_delay,na.rm=T))
#compute the min air_time
summarise(flights,min(air_time,na.rm=T))
#compute mean and standard deviation of the distance per each destination
by_destination <- group_by(flights,dest)
summarise(by_destination,avg_dist=mean(distance,na.rm=T),sd_dist=sd(distance,na.rm=T))
#compute the max and min of air_time per each month
by_month <- group_by(flights,year,month)
summarise(by_month,max_air = max(air_time,na.rm=T),min_air = min(air_time,na.rm = T))
```

### We will cover the remaining sections of chapter 5 in the next class make sure that your are not behind and you understand what we covered so far.
